...
a) To show: Let $a,b \in \mathbb{R}$. The standard ordering $\leq$ on the real numbers is complete on the closed interval $[a,b]$, i.e. $[a,b]$ has a smallest element w.r.t. $\leq$ and for every chain $S  \subseteq [a,b]$ there exists a least upper bound $sup S \in [a,b]$. \\ \\
Proof:\\
The smallest element in $[a,b] = \{ r \vert a \leq r \leq b\}$ with respect to $\leq$ is a, since $a \leq x$ for every $x \in [a,b]$ (this follows from the definition of a closed interval).\\
Since $[a,b]$ is bounded from above by b ($x \leq $ for every $x\in [a,b]$), every chain $S  \subseteq [a,b]$ S is bounded from above as well:\\
Case 1. $S = [c,b] = \{ r \vert c \leq r \leq b\}$ with $ c\in [a,b]$. Then S is bounded from above by b. \\ 
Case 2. $S = [c,d] = \{ r \vert c \leq r \leq d\}$ with $ c,d\in [a,b]$ and $c\leq d$. Then S is bounded from above by d.\\
We know that $\mathbb{R}$ is complete, therefore all chains $S  \subseteq [a,b]$ have a least upper bound with $supS = d$ for $S = [c,d] \subseteq [a,b]$.\\
Thus, the standard ordering $\leq$ on the real numbers is complete on the closed interval $[a,b]$.
\end{document}
